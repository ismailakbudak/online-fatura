...
<?php
    ...
    $musteri_fk = $_GET['musteri_fk'];
    
    if ($sirket_pk) {
          echo ' <select id="musteri_fk" name="musteri_fk" class="musteri_fk"  >
                          <option value="0"></option>'; 
       $db = new musteri_detay();
       $musteriler = $db->listele($sirket_pk);
       if ($musteriler) {
         foreach ($musteriler as  $row) {
               if ($musteri_fk == $row['pk']) {
                      echo "  <option selected value='{$row['pk']}' > {$row['musteri_unvan']} </option> ";             
               }
               else{
